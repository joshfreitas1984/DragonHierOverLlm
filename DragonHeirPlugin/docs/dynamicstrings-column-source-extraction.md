...
`dynamicStrings.txt` (itself fully auto-generated by
`GameFileHandling.ExtractDynamicStringCandidatesFromIl2CppStringMap` from
`Converter/output/_dynamicStrings_candidates.txt` — no manual review/authoring step exists) purely
for traceability
...
TextFileType enum value needed. Run via the merged `"1c. ExportDynamicStringsIntoTranslated"` fact
